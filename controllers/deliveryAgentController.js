...
exports.refreshToken = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        // refresh requests (e.g. the app polling several endpoints, all
        // 401'ing around the same time) to race: the first rotates and
        // saves a new refresh_token, then every other in-flight request's
        // still-valid-by-expiry old token no longer matches, so it gets
        // rejected with 403 and the app force-logs-out the agent. The
        // refresh token stays valid until its own expiry or a fresh login.
