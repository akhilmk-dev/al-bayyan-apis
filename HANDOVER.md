...
- **Deep link**: customer notifications now include an `app_url` of `com.albayan://OrderHistory/<mongo order _id>` (`utils/notifyCustomerStatus.js`) - screen/route name `OrderHistory` is confirmed against the actual customer app's navigation.
