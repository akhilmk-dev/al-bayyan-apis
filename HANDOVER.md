...
- **Credentials**: `ONESIGNAL_APP_ID`/`ONESIGNAL_REST_API_KEY` (agent app) and the new `ONESIGNAL_CUSTOMER_APP_ID`/`ONESIGNAL_CUSTOMER_REST_API_KEY` (customer app) are all still blank. The customer pair falls back to the agent pair if unset — fine for early testing, but the customer app should get registered as its own OneSignal project before production (agent and customer external IDs use different formats - Mongo ObjectId hex vs. numeric Shopify ID - so no collision risk either way).
