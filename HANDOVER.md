...
| `ONESIGNAL_APP_ID`, `ONESIGNAL_REST_API_KEY` | Push notifications to delivery agents (staff app) | ❌ not yet supplied |
...
- **Credentials**: `ONESIGNAL_CUSTOMER_APP_ID`/`ONESIGNAL_CUSTOMER_REST_API_KEY` (customer app, bundle `com.albayan`) are now configured and verified live. `ONESIGNAL_APP_ID`/`ONESIGNAL_REST_API_KEY` (agent/staff app) are still blank - the customer pair no longer needs to fall back to them.
- **Deep link**: customer notifications now include an `app_url` of `com.albayan://OrderDetail/<mongo order _id>` (`utils/notifyCustomerStatus.js`) - mirrors the staff app's `com.albayan_staffapp://OrderDetail/...` convention, but the screen/route name (`OrderDetail`) is a guess based on that convention, not confirmed against the actual customer app - confirm/adjust with whoever owns that codebase.
